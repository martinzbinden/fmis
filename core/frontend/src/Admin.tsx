...
export default function Admin() {
  ...
  return (
    <div className="mx-auto max-w-2xl space-y-8 p-4">
      {canManageModules && <ModulesSection />}
...
function ModulesSection() {
  ...
  async function toggle(m: ModuleInfo) {
    try {
      await setModuleEnabled(m.key, !m.enabled)
      await reload()
    } catch (err) {
  ...
  return (
    <div>
      <h1 className="mb-2 text-xl font-bold text-gray-800">Module</h1>
      {error && <p className="mb-2 text-sm text-red-600">{error}</p>}
      {loading ? (
        <div className="p-4 text-center text-gray-400">Lädt…</div>
      ) : (
